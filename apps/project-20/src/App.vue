    ...
    <div class="flex min-h-screen">
      ...
      <aside class="w-60 shrink-0 hidden md:flex flex-col gap-5 p-5 border-r border-white/10 bg-white/[0.02]">
        <div class="flex items-center gap-2.5">
          <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center"><Store :size="18" /></div>
      ...
      <main class="flex-1 min-w-0 p-5 md:p-7">
        <header class="flex flex-wrap items-center justify-between gap-3 mb-6">
          <div>
            <h1 class="text-2xl font-bold flex items-center gap-2.5">QuantumCrypto Analytics<span class="text-sky-400">Dashboard</span>
            ...
            <div class="flex items-center justify-between p-4 border-b border-white/10">
              ...
              <div class="flex items-center gap-1.5">
                <button v-for="f in ['all', 'low', 'out'] as const" :key="f" @click="activeFilter = f"
                  class="px-3 py-1.5 rounded-lg text-[12px] capitalize" :class="activeFilter === f ? 'bg-violet-500 text-white' : 'bg-white/5 text-slate-400 hover:bg-white/10'">
        ...
        <footer class="mt-6 pt-5 border-t border-white/10 flex flex-wrap items-center justify-between gap-3 text-[12px] text-slate-400">
          ...
          <div class="flex items-center gap-3">
            ...
            <span>QuantumCrypto Analyticsv2.4 · data synced 2 min ago</span>
          </div>
        </footer>
      </main>
    </div>
    ...
    <transition name="fade">
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md rounded-2xl bg-[#0d0d1a] border border-white/10 p-5">
          ...
          <button @click="placeOrder" class="mt-4 w-full py-2.5 rounded-xl bg-violet-500 text-white text-sm font-semibold hover:bg-violet-400">Submit purchase order</button>
        </div>
      </div>
    </transition>
