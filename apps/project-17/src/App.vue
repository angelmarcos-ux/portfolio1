    ...
    <div class="flex">
      ...
      <aside class="w-60 shrink-0 hidden md:flex flex-col gap-5 p-5 border-r border-white/10 bg-white/[0.02] sticky top-0 h-screen">
        ...
        <nav class="flex flex-col gap-1 text-sm">
          <button v-for="v in views" :key="v.key" @click="currentView = v.key"
            ...
            :class="currentView === v.key ? 'bg-indigo-500/15 text-indigo-300 font-medium' : 'text-slate-400 hover:bg-white/5 hover:text-slate-200'">
      ...
      <main class="flex-1 min-w-0 p-5 md:p-7 pb-10">
        <header class="flex flex-wrap items-center justify-between gap-3 mb-6">
          <div>
            <h1 class="text-2xl font-bold flex items-center gap-2.5 flex-wrap">RetailOps<span class="text-indigo-400">{{ titles[currentView] }}</span>
        ...
        <nav class="md:hidden flex gap-1.5 overflow-x-auto pb-3 -mx-1 px-1 mb-4">
          <button v-for="v in views" :key="v.key" @click="currentView = v.key"
            ...
            :class="currentView === v.key ? 'bg-indigo-500/15 text-indigo-300 font-medium border border-indigo-500/20' : 'bg-white/5 text-slate-400 border border-white/10 hover:bg-white/10'">
