    ...
    <div class="flex min-h-screen">
      ...
      <aside class="w-60 shrink-0 hidden md:flex flex-col gap-5 p-5 border-r border-white/10 bg-white/[0.02]">
        <div class="flex items-center gap-2.5">
          <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-500 flex items-center justify-center"><Store :size="18" /></div>
          <div><p class="font-bold leading-tight">RetailOps</p><p class="text-[11px] text-slate-400">Control Tower</p></div>
        </div>
        <nav class="flex flex-col gap-1 text-sm">
          <span class="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-indigo-500/15 text-indigo-300 font-medium"><BarChart3 :size="16" /> Overview</span>
      ...
      <main class="flex-1 min-w-0 p-5 md:p-7">
        <header class="flex flex-wrap items-center justify-between gap-3 mb-6">
          <div>
            <h1 class="text-2xl font-bold flex items-center gap-2.5">RetailOps <span class="text-indigo-400">Dashboard</span>
        ...
        <section class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6">
          <div class="rounded-2xl p-4 bg-white/5 border border-white/10">
            <div class="flex items-center justify-between"><span class="text-[13px] text-slate-400">Today Sales</span><span class="p-1.5 rounded-lg bg-indigo-500/10 text-indigo-400"><TrendingUp :size="16" /></span></div>
              ...
              <table class="w-full text-sm">
                ...
                <tbody>
                  <tr v-for="r in filtered" :key="r.sku" class="border-b border-white/5 hover:bg-white/[0.03]">
                    ...
                    <td class="px-4 py-3 text-right"><button @click="toast(`Reorder queued for ${r.name}`)" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-500/15 text-indigo-300 text-[12px] hover:bg-indigo-500/25"><PackagePlus :size="13" /> Reorder</button></td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <footer class="mt-6 pt-5 border-t border-white/10 flex flex-wrap items-center justify-between gap-3 text-[12px] text-slate-400">
          ...
          <div class="flex items-center gap-3">
            ...
            <span>RetailOps v2.4 · data synced 2 min ago</span>
          </div>
        </footer>
      </main>
    </div>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md rounded-2xl bg-[#0d0d1a] border border-white/10 p-5">
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-2"><ScanLine :size="18" class="text-indigo-400" /><h3 class="font-semibold">Quick Reorder</h3></div>
          ...
          <button @click="placeOrder" class="mt-4 w-full py-2.5 rounded-xl bg-indigo-500 text-white text-sm font-semibold hover:bg-indigo-400">Submit purchase order</button>
        </div>
      </div>
    </transition>
