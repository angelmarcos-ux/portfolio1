        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <div class="flex items-center gap-1 p-1 rounded-xl bg-black/30 border border-white/10">
            <button v-for="t in ['all', 'pending', 'shipped', 'delivered'] as const" :key="t" @click="tab = t"
              class="px-3 py-1.5 rounded-lg text-[12px] capitalize" :class="tab === t ? 'bg-indigo-500 text-white' : 'text-slate-400 hover:bg-white/10'">
              {{ t }} ({{ countFor(t) }})
            </button>
          </div>
        </div>
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-2 min-w-28">
                  <div class="flex-1 h-1.5 rounded-full bg-white/10">
                    <div class="h-1.5 rounded-full bg-gradient-to-r from-sky-500 to-indigo-400" :style="{ width: o.progress + '%' }"></div>
                  </div>
