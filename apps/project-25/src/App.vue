    ...
    <div class="flex min-h-screen">
      ...
      <main class="flex-1 min-w-0 p-5 md:p-7">
        <header class="flex flex-wrap items-center justify-between gap-3 mb-6">
          <div>
            <h1 class="text-2xl font-bold flex items-center gap-2.5">HorizonXR Collaborative<span class="text-sky-400">Dashboard</span>
            ...
            <div class="flex items-center justify-between p-4 border-b border-white/10">
              ...
              <div class="flex items-center gap-1.5">
                <button v-for="f in ['all', 'low', 'out'] as const" :key="f" @click="activeFilter = f"
                  class="px-3 py-1.5 rounded-lg text-[12px] capitalize" :class="activeFilter === f ? 'bg-teal-500 text-white' : 'bg-white/5 text-slate-400 hover:bg-white/10'">
        ...
        <footer class="mt-6 pt-5 border-t border-white/10 flex flex-wrap items-center justify-between gap-3 text-[12px] text-slate-400">
          ...
          <div class="flex items-center gap-3">
            ...
            <span>HorizonXR Collaborativev2.4 · data synced 2 min ago</span>
          </div>
        </footer>
      </main>
    </div>
    ...
    <transition name="fade">
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md rounded-2xl bg-[#0d0d1a] border border-white/10 p-5">
          ...
          <button @click="placeOrder" class="mt-4 w-full py-2.5 rounded-xl bg-teal-500 text-white text-sm font-semibold hover:bg-teal-400">Submit purchase order</button>
        </div>
      </div>
    </transition>
